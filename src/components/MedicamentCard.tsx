    ...
    <div className="custom-card" onClick={() => setIsExpanded(!isExpanded)}>
      ...
      {isExpanded && (
        <div className="expanded-info">
          <hr />
          <p><span className="material-icons">euro_symbol</span> Prix: {medicament[2].toLocaleString('fr-HT', { style: 'currency', currency: 'HTG' })}</p>
          <p><span className="material-icons">category</span> Type: {medicament[3]}</p>
          <p><span className="material-icons">local_offer</span> Présentation: {medicament[4]}</p>
        </div>
      )}
    </div>
  );
